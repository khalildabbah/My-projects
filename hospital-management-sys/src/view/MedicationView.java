...
public class MedicationView extends JPanel implements Callback, ActionListener, IView {
	...
	private int index = 0;
	private Map<Integer, Medication> meds = new HashMap<>();
	...
	public MedicationView() {
		...
		JLabel lblNewLabel = new JLabel("Medication");
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == removeBTN) {
			if (table.getSelectedRowCount() == 1) {
				...
				Medication prob = meds.get(index);
				if (prob != null && Hospital.getInstance().removeMedication(prob)) {
					updateTable();
					JOptionPane.showMessageDialog(this, "Medication is removed!");
				} else {
					JOptionPane.showMessageDialog(this, "Unable to remove the medication!");
				}
			} else {
				JOptionPane.showMessageDialog(this, "Select only 1 row from table.");
			}
		} else if (e.getSource() == addBTN) {
			new AddMedication(Operation.ADD, this).setVisible(true);
			...
			if (table.getSelectedRowCount() == 1) {
				...
				Medication prob = meds.get(index);
				if (prob != null) {
					new AddMedication(Operation.VIEW, prob, null).setVisible(true);
				} else {
	...
	private void updateTable() {
		...
		index = 0;
		meds.clear();
		for (Medication each: Hospital.getInstance().getMedications().values()) {
			tableData.add(each.getTableValues());
			meds.put(index++, each);
		}
		table = new JTable(tableData.toArray(new String[][] {}), 
				Medication.getTableNames());
	...
	public void callback(Object data) {
		if (data instanceof Medication) {
			try {
				if (!Hospital.getInstance().addMedication((Medication) data)) {
					JOptionPane.showMessageDialog(this, "Unable to add the medication.");
				} else {
					meds.put(index++, (Medication) data);
				}
			} catch (Exception ex) {
