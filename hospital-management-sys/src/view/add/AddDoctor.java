...
public class AddDoctor extends FrameParent {
	...
	public AddDoctor(Operation op, Callback call) {
		this (op, null, call);
	}
	
	public AddDoctor(Operation op, Doctor doctor, Callback call) {
		super("Doctor", 300, 580, false);
		contentPane = super.getMainPane();
		this.operation = op;
		this.doctor = doctor;
		this.callback = call;
		init();
		if (op == Operation.VIEW && doctor != null) {
			idF.setText(doctor.getId() + "");
			fnameF.setText(doctor.getFirstName());
			lnameF.setText(doctor.getLastName());
			birthF.setText(UtilsMethods.formatDate(doctor.getBirthDate()));
			addressF.setText(doctor.getAddress());
			phoneF.setText(doctor.getPhoneNumber());
			emailF.setText(doctor.getEmail());
			genderF.setText(doctor.getGender());
			workF.setText(UtilsMethods.formatDate(doctor.getWorkStartDate()));
			salaryF.setText(doctor.getSalary() + "");
			passwordF.setText(doctor.getPassword());
			specialF.setSelectedItem(doctor.getSpecialization());
			internshipF.setSelected(doctor.isFinishInternship());
			licenseF.setText(doctor.getLicenseNumber() + "");
	...
	private void init() {
		...
		internshipF = new JCheckBox("Yes");
		internshipF.setBackground(new Color(144, 238, 144));
		internshipF.setBounds(124, 383, 97, 23);
		contentPane.add(internshipF);
		
		specialF = new JComboBox<>();
		for (Specialization sp: Specialization.values()) {
			specialF.addItem(sp);
		}
		specialF.setBounds(124, 413, 150, 22);
		...
		profileBTN.setBounds(124, 443, 150, 22);
		contentPane.add(profileBTN);
		
		profileF = new JTextField();
		profileF.setBounds(10, 473, 264, 22);
		...
		submitBTN.setBounds(10, 505, 104, 23);
		...
		JLabel lblInternshipDone = new JLabel("Internship Done");
		lblInternshipDone.setBounds(10, 387, 104, 14);
		contentPane.add(lblInternshipDone);
		
		JLabel lblSpecialization = new JLabel("Specialization:");
		lblSpecialization.setBounds(10, 417, 104, 14);
		contentPane.add(lblSpecialization);

		JLabel lblProfile = new JLabel("Profile Picture:");
		lblProfile.setBounds(10, 447, 104, 14);
				...
				if (idF.getText().isBlank() || fnameF.getText().isBlank() || 
					...
					try { 
						...
						UtilsMethods.resizeAndCopyImage(new File(profileF.getText()), 
								new File(fileName));
						Doctor newDoctor = new Doctor(Integer.parseInt(idF.getText()), fnameF.getText(), 
								...
								internshipF.isSelected(), (Specialization)specialF.getSelectedItem());
						newDoctor.setProfilePicture(fileName);
						if (operation == Operation.UPDATE) {
							Hospital.getInstance().removeDoctor(doctor);
							Hospital.getInstance().addDoctor(newDoctor);
							MainFrame.setStaff(newDoctor);
							JOptionPane.showMessageDialog(this, "Doctor is updated!");
						} else {
							callback.callback(newDoctor);	
							JOptionPane.showMessageDialog(this, "Doctor is added!");
						}
						dispose();
					} catch (Exception ex) {
