...
public class ManageManufacturers extends JPanel {
	...
	public ManageManufacturers() {
		...
		edit_btn = new CustomButton("Edit",150,50,40);
		edit_btn.setFont(new Font("Arial", Font.BOLD, 18));
		edit_btn.addActionListener(e -> enableEditting());
        edit_btn.setBounds(656, 10, 89, 23);
        add(edit_btn);
        
        confirm_btn = new CustomButton("Edit", 150, 50, 40);
        confirm_btn.setFont(new Font("Arial", Font.BOLD, 18));
        confirm_btn.setText("Confirm");
        confirm_btn.setBounds(656, 10, 89, 23);
        confirm_btn.setVisible(false);
        confirm_btn.addActionListener(e -> confirmChanges());
        add(confirm_btn);
        
        CustomButton delete_btn = new CustomButton("Edit", 150, 50, 40);
        delete_btn.setFont(new Font("Arial", Font.BOLD, 18));
        delete_btn.setText("Delete");
        delete_btn.setBounds(656, 36, 89, 23);
        delete_btn.addActionListener(e -> deleteMan());
        add(delete_btn);
	...
	private void enableEditting() {
		...
		wineName_tf.setEditable(true);
		description_tf.setEditable(true);
		prodYear_tf.setEditable(true);
		price_tf.setEditable(true);
		sweetness_tf.setEditable(true);
	...
	private void confirmChanges() {
		...
		if(man!=null) {
			man.setName(name_TextField.getText());
			man.setPhone(phoneNumber_TextField.getText());
			man.setAddress(address_TextField.getText());
			man.setEmail(email_TextField.getText());
		}
		if(bottle!=null) {
			bottle.setName(wineName_tf.getText());
			bottle.setDescription(description_tf.getText());
			bottle.setProductionYear(Integer.valueOf(prodYear_tf.getText()));
			bottle.setPricePerBottle(Double.valueOf(price_tf.getText()));
			bottle.setSweetnessLevel(sweetness_tf.getText());
		}
        ...
        DatabaseController.updateManufacturer(man);
        if(bottle!=null) {
           DatabaseController.updateWineBottle(bottle);
        }
	...
	private void deleteMan() {
	    ...
	    int response = JOptionPane.showConfirmDialog(null, 
	            "Are you sure you want to delete this Manufacturer and all his wine bottles?", 
	            "Confirm Deletion", 
	    ...
	    if (response == JOptionPane.YES_OPTION) {
			Manufacturer man = (Manufacturer)comboBox.getSelectedItem();
			if(man!=null)
				DatabaseController.deleteManufacturer(man.getManufacturerId());
		    for(WineBottle bottle : DatabaseController.getAllWineBottles().values()) 
		    	if(bottle.getManufacturerID()==man.getManufacturerId()) {
		    		DatabaseController.deleteWineBottle(bottle.getWineSerialNum());
		    }
		    
    		listModel_1.removeAllElements();
			comboBox.removeItem(man);
		    displayWineDetails();
		    displayManDetails();
